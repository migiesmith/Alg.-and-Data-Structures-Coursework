		...
		while(overCap){
			...
			for(int i = 0; i < circles.size(); i++){
				...
				if(c.demand > prob.depot.c){
					...
					Circle newC = new Circle( new Point2D.Double(mx,my), c.radius);
					*/
					
					Circle newC = new Circle(farthest, c.radius);
		...
		while(merged){
			...
			for(int i = 0; i < circles.size(); i++){
				for(int j = 0; j < circles.size(); j++){
					...
					if((ci.demand + cj.demand <= prob.depot.c) && (ci.pos.distance(cj.pos) <= ci.radius + cj.radius)){
						circles.remove(cj);
						ci.mergeRoutes(cj);
						ci.shrink();
					}
		...
		for(Circle c : this.circles){
			String disk = String.format(
					"<g transform='translate(%.0f,%.0f)'>"+
							...
							"<text text-anchor='middle' y='5'>%d</text>"+
							"</g>\n", 
							c.pos.x, c.pos.y, (int)c.radius, 0);
			psb.append(disk);
			ssb.append(disk);
		}
